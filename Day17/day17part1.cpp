...
/*   Updated: 2020/12/17 16:52:03 by tbruinem      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	add_neighbours(map<tuple<int, int, int>, int>&	neighbours, const tuple<int, int, int>& cur)
{
	for (int i = -1; i <= 1; i++)
	{
		for (int j = -1; j <= 1; j++)
		{
			for (int k = -1; k <= 1; k++)
			{
				if (i == 0 && j == 0 && k == 0)
					continue ;
				neighbours[std::make_tuple(std::get<0>(cur) + i, std::get<1>(cur) + j, std::get<2>(cur) + k)]++;
			}
		}
	}
}

set<tuple<int, int, int>>	new_cycle(set<tuple<int, int, int>> previous)
{
	map<tuple<int, int, int>, int>	active_neighbours;
...
int main(void)
{
	...
	for (size_t i = 0; !cin.eof(); i++)
	{
		...
		{
			if (line[j] == '#')
				active.insert(tuple<int, int, int>(j, i, 0));
		}
	}
